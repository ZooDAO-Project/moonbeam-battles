...
@pytest.fixture(scope="module")
def staking(NftStakingPosition, listing, tokens, accounts):
	(zooToken, daiToken, linkToken, nft) = tokens
	listingList = listing[0]

	return NftStakingPosition.deploy("zStakerPosition", "ZSP", listingList.address, zooToken.address, {"from": accounts[0]})

@pytest.fixture(scope="module")
def voting(NftVotingPosition, tokens, accounts):
	(zooToken, daiToken, linkToken, nft) = tokens

	return NftVotingPosition.deploy("zVoterPosition", "ZVP", daiToken.address, zooToken.address, {"from": accounts[0]})
...
def battles(accounts, tokens, vault, listing, xZoo, staking, voting, jackpotA, jackpotB, base_zoo_functions, token_controller, well, wGlmr, ZooGovernance, NftBattleArena):
	...
	listingList.voteForNftCollection(nft.address, 1e20, listingList.maxTimelock())
	zooToken.transfer(staking, arena.baseStakerReward() * 100)

	return (vault, base_zoo_functions, governance, staking, voting, arena, listingList, xZoo, jackpotA, jackpotB)
...
def second_stage(accounts, tokens, battles):
	...
	return ((zooToken, daiToken, linkToken, nft), (vault, functions, governance, staking, voting, arena, listingList, xZoo, jackpotA, jackpotB))
...
def fourth_stage(accounts, third_stage):
	...
	(vault, functions, governance, staking, voting, arena, listing, xZoo, jackpotA, jackpotB) = third_stage[1]
...
def fifth_stage(accounts, fourth_stage):
	...
	(vault, functions, governance, staking, voting, arena, listing, xZoo, jackpotA, jackpotB) = fourth_stage[1]
